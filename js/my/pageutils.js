    ...
    $(function () {
        $.ajax({
            ...
            success: function (data) {
                ...
                console.log("Successfully fetch user info.")
            }
        })
    })
}
...
function loadUserDetails() {
    ...
    $(function () {
        $.ajax({
            ...
            success: function (data) {
                var obj = eval(data);
                var role = "b1f707d4-bff4-4670-a0e4-21ddc812bf69" //Regular User
                if (obj.rule != 20) {
                    role = "924a86a5-58b2-4158-b487-a567040f8df8" //Admin
                    document.getElementById("adminManage").style.display = ""
                }
                document.getElementById("myUsername").value = obj.username;
                document.getElementById("myReaderId").value = obj.id;
                document.getElementById("myEmail").value = obj.email;
                document.getElementById("myBorrowSum").value = obj.rule;
                if (role == "b1f707d4-bff4-4670-a0e4-21ddc812bf69") {
                    document.getElementById("myRole").value = "普通读者";
                } else if (role == "924a86a5-58b2-4158-b487-a567040f8df8") {
                    document.getElementById("myRole").value = "管理员";
                }
                var str = "{username:'" + obj.username + "',userid:'" + obj.id +";,userrole:'" + role + "'}"
                var base = new Base64();
                userString = base.encode(str);
            }
        })
    })
}
